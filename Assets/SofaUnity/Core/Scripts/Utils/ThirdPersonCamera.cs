...
namespace SofaUnity
{
	...
	public class ThirdPersonCamera : MonoBehaviour
	{
		...
		private float m_currentY = 42.0f;
		...
		void Start()
		{
			...
			{
				// Derive the orbit look-at point and angles from whatever position/rotation
				// the Camera Transform was left at in the editor, so Play mode reproduces
				// the exact same framing instead of snapping to a hardcoded default angle.
				if (m_cameraDistance <= 0.0f)
					m_cameraDistance = 100.0f;

				m_lookAtStatic = transform.position + transform.forward * m_cameraDistance;
			}

			Vector3 dir = -transform.forward;
			m_currentY = -Mathf.Asin(Mathf.Clamp(dir.y, -1f, 1f)) * Mathf.Rad2Deg;
			m_currentX = Mathf.Atan2(-dir.x, -dir.z) * Mathf.Rad2Deg;
		}
